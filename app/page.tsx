...
import { DEFAULT_HEADER_STATE } from "./_data/HeaderPresets";
...
import { SECTIONS, type SectionId, type HeaderState, type StudioPreset } from "./types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<HeaderState>(DEFAULT_HEADER_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const [activePresetId, setActivePresetId] = useState<string | null>(null);
  const [downloadName] = useState("headers-component");
  ...
  const update = <K extends keyof HeaderState>(key: K, value: HeaderState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_HEADER_STATE, ...(preset.state as Partial<HeaderState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Header Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
